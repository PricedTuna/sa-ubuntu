...
# Función para configurar una IP fija
function set_static_ip() {
    ...
    if ! validate_ip "$ip_address"; then
        PrintMessage "⚠️ Error: La dirección IP ingresada no es válida." "error"
        return
    fi
...
EOF
    ...
    if sudo netplan apply; then
        PrintMessage "✅ IP configurada correctamente en $interface con dirección $ip_address" "info"
    else
        PrintMessage "❌ Error al aplicar la configuración. Restaurando respaldo..." "error"
